...
// (env vars aren't available during Vercel preview builds)
...
export async function GET() {
  ...
  try {
    ...
    const { data: dreamEntryColumns, error: columnsError } =
      await supabaseAdmin.rpc("get_column_info", {
    ...
    await supabase
      .from("profile")
      .insert([{ user_id, language: "English", bible_version: "KJV" }]);
      ...
      .insert([
        {
          user_id,
          original_text:
            "I was flying above the city, but then fell into water.",
          title: "Flying and Falling",
          dream_summary: "A dream about gaining freedom and losing control.",
          analysis_summary:
            "This dream signifies a struggle between ambition and self-doubt.",
          topic_sentence:
            "Sometimes, our greatest aspirations carry the risk of failure.",
    ...
    const { data: testDreamDirect, error: testDreamDirectError } =
      await supabaseAdmin
        .from("dream_entries")
        .insert({
          user_id,
