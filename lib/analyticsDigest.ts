...
function digestEmail(
  ...
  // v3 "Deep Current" shell. This was the last surviving Moonwater surface in
  // the codebase — a warm-paper body, a metallic hairline rule, an italic
  // Georgia wordmark, and a three-stop gradient header, all of which the v3
  // rules retire (that whole palette is deprecated; the gradient belongs to
  // the logo alone; the wordmark is Quicksand, never italic serif). It
  // escaped the §2 grep gate only because that gate scopes to app/ and
  // components/ — hence no retired hex values quoted here either.
  //
  // Email constraints shape the port: no CSS variables (Outlook), no webfonts
  // (most clients), so the tokens are inlined as literals and the wordmark
  // falls back to a rounded system stack — the same compromise the
  // transactional templates in lib/emails/templates.ts already make. Flat
  // Navy 900 header, Indigo rule, Paper body, Mist hairlines.
  const rowsHtml = rows
    .map(
      (r) => `
        <tr>
          <td style="padding:11px 0; border-bottom:1px solid #E4E1F3; font-family:Georgia,'Times New Roman',serif; font-size:15px; color:#575C88;">
            ${r.label}
            ${r.delta ? `<div style=\"font-family:Helvetica,Arial,sans-serif; font-size:11px; color:#575C88; padding-top:3px;\">${r.delta}</div>` : ""}
          </td>
          <td align="right" style="padding:11px 0 11px 16px; border-bottom:1px solid #E4E1F3; font-family:Georgia,'Times New Roman',serif; font-size:17px; color:#0A0E33; white-space:nowrap; vertical-align:top;">
  ...
  const html = `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>DreamRiver analytics</title></head><body style="margin:0; padding:0; background-color:#FBFAFF;"><div style="display:none; max-height:0; overflow:hidden; mso-hide:all;">Your ${cadenceLabel} DreamRiver numbers, ${periodLabel}.&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;</div><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#FBFAFF;"><tr><td align="center" style="padding:36px 16px;"><table role="presentation" width="560" cellpadding="0" cellspacing="0" border="0" style="width:560px; max-width:100%; background-color:#FFFFFF; border:1px solid #E4E1F3; border-radius:14px; overflow:hidden;"><tr><td align="center" bgcolor="#0A0E33" style="background-color:#0A0E33; padding:30px 40px 26px;"><div style="font-family:'Trebuchet MS', Verdana, sans-serif; font-size:26px; font-weight:bold; letter-spacing:0.02em; color:#F0EFFC; line-height:1.2;"><span style="color:#B39BFF;">&#10022;</span>&nbsp;DreamRiver</div><div style="padding-top:10px;"><table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center"><tr><td width="44" height="2" bgcolor="#B39BFF" style="font-size:0; line-height:0;">&nbsp;</td></tr></table></div><div style="font-family:Helvetica,Arial,sans-serif; font-size:10px; letter-spacing:0.28em; color:#A7ADD6; text-transform:uppercase; padding-top:12px;">Founders&rsquo; analytics</div></td></tr><tr><td style="padding:36px 44px 6px;"><div style="font-family:Helvetica,Arial,sans-serif; font-size:11px; letter-spacing:0.24em; text-transform:uppercase; color:#575C88;">${periodLabel}</div><div style="font-family:Georgia,'Times New Roman',serif; font-size:28px; line-height:1.25; color:#0A0E33; padding-top:10px;">The ${cadenceLabel} pulse.</div></td></tr><tr><td style="padding:16px 44px 30px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">${rowsHtml}</table></td></tr><tr><td bgcolor="#F3F0FE" style="padding:20px 44px 24px; border-top:1px solid #E4E1F3;"><div style="font-family:Helvetica,Arial,sans-serif; font-size:11.5px; line-height:1.7; color:#575C88;">Change what&rsquo;s included, the cadence, or the send time in <a href="https://www.dreamriver.io/admin/system" style="color:#2036BE; text-decoration:underline;">Admin &rarr; System &rarr; Analytics digest</a>.</div></td></tr></table><table role="presentation" width="560" cellpadding="0" cellspacing="0" border="0" style="width:560px; max-width:100%;"><tr><td align="center" style="padding:14px 20px 0;"><div style="font-family:Helvetica,Arial,sans-serif; font-size:10.5px; letter-spacing:0.18em; text-transform:uppercase; color:#575C88;">sent from send.dreamriver.io</div></td></tr></table></td></tr></table></body></html>`;
